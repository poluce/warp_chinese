...
const MODAL_SUBHEADER: &str =
    "根据从图像（.png、.jpg）中提取的颜色自动生成主题。";
const IMAGE_PICKER_BUTTON_PRE_SELECT_TEXT: &str = "选择图像";
const IMAGE_PICKER_BUTTON_SELECTING_TEXT: &str = "正在选择图像...";
const IMAGE_PICKER_BUTTON_POST_SELECT_TEXT: &str = "选择新图像";
...
impl ThemeCreatorBody {
    ...
    pub fn create_theme(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(theme_options) = self.theme_options.as_mut() {
            ...
            if errored {
                self.send_error_toast("出现问题".to_string(), ctx);
            }
        }
    }
