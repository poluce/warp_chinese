...
const NEXT_COMMAND_DESCRIPTION: &str = "让 AI 根据命令历史、输出和常用工作流建议下一条要运行的命令。";
const PROMPT_SUGGESTIONS_DESCRIPTION: &str = "让 AI 根据最近的命令及其输出，在输入框中以内联横幅形式建议自然语言提示词。";
const SUGGESTED_CODE_BANNERS_DESCRIPTION: &str = "让 AI 根据最近的命令及其输出，在块列表中以行内横幅形式建议代码差异和查询。";
const NATURAL_LANGUAGE_AUTOSUGGESTIONS: &str =
    "让 AI 根据最近的命令及其输出建议自然语言自动补全。";
const SHARED_BLOCK_TITLE_GENERATION_DESCRIPTION: &str =
    "让 AI 根据命令和输出为共享块生成标题。";
const GIT_OPERATIONS_AUTOGEN_DESCRIPTION: &str =
    "让 AI 生成提交消息和拉取请求标题及描述。";
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let conversation_layout_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            match current {
                ...
                OpenConversationPreference::SplitPane => {
                    dropdown.set_selected_by_name("分屏", ctx)
                }
            };
            dropdown
        });
    ...
    fn render(
        ...
        if FeatureFlag::ProfilesDesignRevamp.is_enabled() {
            ...
            agents_header.add_child(render_ai_setting_description(
                "设置 Agent 的操作边界。选择它可以访问的内容、拥有多少自主权，以及何时必须请求您的批准。您还可以微调有关自然语言输入、代码库感知等方面的行为。",
                ai_settings.is_any_ai_enabled(app),
                app,
            ));
...
impl AgentsWidget {
            ...
            .with_child(
                Container::new(
                    render_ai_setting_description(
                        "配置文件可让您定义 Agent 的操作方式——从它可以执行的操作和何时需要批准，到用于编码和规划等任务的模型。您还可以将其限定到单个项目。",
                        is_any_ai_enabled,
                        app,
                    )
                )
    ...
    fn render_command_denylist(
        ...
        render_ai_list(
            "命令黑名单",
            "匹配 Warp Agent 始终需要请求执行权限的命令的正则表达式。",
            list,
            view,
            ai_settings,
            app,
    ...
    fn render_base_model_setting(
        ...
        render_dropdown_item(
            ...
            "基础模型",
            Some(
                "此模型作为 Warp Agent 的主要引擎，驱动大多数交互，并在必要时调用其他模型进行规划和代码生成等任务。Warp 可能会根据模型可用性或辅助任务（如对话摘要）自动切换到备用模型。",
            ),
    ...
    fn render_mcp_permissions_zero_state(
        ...
        let subtext = {
            let subtext_fragments = vec![
                FormattedTextFragment::plain_text(
                    "您尚未添加任何 MCP 服务器。添加后，您将能够控制 Warp Agent 与它们交互时的自主程度。",
                ),
    ...
    fn render_natural_language_detection_section(
        ...
        if FeatureFlag::AgentView.is_enabled() {
            static AUTODETECTION_DESCRIPTION_FRAGMENTS: LazyLock<Vec<FormattedTextFragment>> =
                LazyLock::new(|| {
                    vec![
                        FormattedTextFragment::plain_text("遇到错误的检测？"),
                        FormattedTextFragment::hyperlink(
                            "请告诉我们",
            ...
            > = LazyLock::new(|| {
                vec![
                    FormattedTextFragment::plain_text(
                        "启用自然语言检测后，当在终端输入中写入自然语言时将进行检测，然后自动切换到 Agent 模式进行 AI 查询。",
                    ),
                    FormattedTextFragment::plain_text(
                        "遇到错误的输入检测？",
                    ),
                ...
                Flex::column()
                    ...
                    .with_child({
                        ...
                        > = LazyLock::new(|| {
                            vec![
                                FormattedTextFragment::plain_text(
                                    "从全局范围的第三方 AI Agent 配置文件（例如主目录中）自动检测并启动 MCP 服务器。仓库内检测到的服务器不会自动启动，必须从 MCP 设置页面单独启用。",
                                ),
                                FormattedTextFragment::hyperlink(
                                    "查看支持的提供商。",
...
impl AIFactWidget {
    ...
    fn render_warp_drive_context_toggle(
        ...
        let description = render_ai_setting_description(
            "Warp Agent 可以利用您的 Warp Drive 内容，根据您个人和团队的开发者工作流和环境来定制响应。这包括任何工作流、笔记本和环境变量。",
            ai_settings.is_any_ai_enabled(app),
            app,
        );
    ...
    fn render(
        ...
        if FeatureFlag::AgentView.is_enabled() {
            let mut agent_view_column = Flex::column()
                ...
                .with_child(render_ai_setting_description(
                    "在长时间运行的命令中显示使用\"完全终端访问\"Agent 的提示。",
                    is_toggleable,
                    app,
                ));
    ...
    fn render(
        ...
        if is_footer_enabled {
                ...
                let auto_show_toggle_label = render_body_item_label::<AISettingsPageAction>(
                    ...
                    Some(AdditionalInfo {
                        ...
                        tooltip_override_text: Some(
                            "需要您的编码 Agent 的 Warp 插件".to_owned(),
                        ),
                    }),
                    LocalOnlyIconState::for_setting(
            ...
            let command_list_description = appearance
                .ui_builder()
                .paragraph(
                    "添加正则表达式模式，为匹配的命令显示编码 Agent 工具栏。",
                )
                .with_style(UiComponentStyles {
                    font_size: Some(appearance.ui_font_size()),
...
impl SettingsWidget for CloudAgentComputerUseWidget {
    ...
    fn render(
        ...
        let mut column = Flex::column()
            ...
            .with_child(toggle_row)
            .with_child(render_ai_setting_description(
                "在从 Warp 应用启动的云端 Agent 对话中启用计算机使用。",
                !is_disabled,
                app,
            ));
        ...
        if FeatureFlag::Orchestration.is_enabled() {
            ...
            column.add_child(render_ai_setting_description(
                "启用多 Agent 编排，允许 Agent 生成并协调并行子 Agent。",
                is_any_ai_enabled,
                app,
            ));
        }
...
impl ApiKeysWidget {
    ...
    fn render_api_keys_section(
        ...
        let mut column = Flex::column()
            ...
            .with_child(
                Container::new(
                    render_ai_setting_description(
                        "使用您自己的模型提供商 API 密钥供 Warp Agent 使用。API 密钥存储在本地，永远不会同步到云端。使用自动模型或未提供 API 密钥的提供商的模型将消耗 Warp 积分。",
                        is_enabled,
                        app,
                    ))
                ...
                if team.billing_metadata.customer_type == CustomerType::Enterprise {
                    ...
                    if has_admin_permissions {
                        vec![
                            ...
                            FormattedTextFragment::plain_text(" 以使用您自己的 API 密钥。"),
                        ]
                    } else {
                        vec![FormattedTextFragment::plain_text(
                            "请您的团队管理员升级到 Build 计划以使用您自己的 API 密钥。",
                        )]
                    }
                }
            } else {
    ...
    fn render_can_use_warp_credits_with_byok_toggle(
        ...
        let description = render_ai_setting_description(
            "启用后，Agent 请求可能会在出错时路由到 Warp 提供的模型之一。Warp 将优先使用您的 API 密钥而非 Warp 积分。",
            ai_settings.is_any_ai_enabled(app),
            app,
        );
...
impl AwsBedrockWidget {
    ...
    fn render_aws_bedrock_section(
        ...
        let toggle_description = if is_admin_enforced {
            "Warp 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。此设置由您的组织管理。".to_string()
        } else {
            "Warp 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。"
                .to_string()
        };
        ...
        let description = render_ai_setting_description(
            "启用后，登录命令将在 AWS Bedrock 凭证过期时自动运行。",
            is_usage_enabled,
            app,
        );
        column.add_child(
