...
const CODEBASE_INDEX_DESCRIPTION: &str = "Warp 可以在您浏览代码仓库时自动索引，帮助代理快速理解上下文并提供解决方案。代码永远不会存储在服务器上。如果代码库无法索引，Warp 仍可通过 grep 和 find 工具调用浏览代码库并获取洞察。";
const WARP_INDEXING_IGNORE_DESCRIPTION: &str = "要从索引中排除特定文件或目录，请将它们添加到仓库目录中的 .warpindexingignore 文件中。这些文件仍可供 AI 功能访问，但不会包含在代码库嵌入中。";
const AUTO_INDEX_FEATURE_NAME: &str = "默认索引新文件夹";
const AUTO_INDEX_DESCRIPTION: &str = "启用后，Warp 会在您浏览代码仓库时自动索引 - 帮助代理快速理解上下文并提供针对性解决方案。";
...
const INDEXING_DISABLED_GLOBAL_AI_TEXT: &str =
    "必须启用 AI 功能才能使用代码库索引。";
const CODEBASE_INDEX_LIMIT_REACHED: &str = "您的套餐已达到最大代码库索引数。请删除现有索引以自动索引新代码库。";
...
impl CodeSubpage {
    ...
    pub fn title(&self) -> &'static str {
        match self {
            Self::Indexing => "代码库索引",
            Self::EditorAndCodeReview => "编辑器和代码审查",
        }
    }
}
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            ...
            appearance
                ...
                .finish(),
            Some("启用此设置后，代码审查面板将在对话的第一个接受的差异时打开。".into()),
        )
    }
}
