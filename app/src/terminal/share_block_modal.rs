...
const DEFAULT_EMBED_TITLE: &str = "嵌入的 warp 块";
const BLOCK_CREATION_FAILED_MESSAGE: &str = "出现问题，请重试。";
...
impl ShareBlockModal {
    ...
    fn render_manage_permalinks_button(&self, appearance: &Appearance) -> Box<dyn Element> {
        let mut button = appearance
            ...
            .with_centered_text_label("管理共享代码块".to_string())
            .with_style(
                self.button_style_overrides(appearance)
                    .set_font_size(12.)
                    .set_padding(Coords {
    ...
    fn render_modal(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        if !link_generated {
            ...
            let redact_secrets_description = appearance
                .ui_builder()
                .span("编辑密钥（API 密钥、密码、IP 地址、个人身份信息等）".to_string())
                .build()
                .with_margin_left(4.)
                .finish();
            column.add_child(
