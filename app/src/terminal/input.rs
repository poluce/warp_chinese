...
const AGENT_MODE_HINT_OPTIONS: &[&str] = &[
    "Warp 任意内容，例如：Deploy my React app to Vercel and set up environment variables",
    "Warp 任意内容，例如：Help me debug why my Python tests are failing in CI",
    "Warp 任意内容，例如：Set up a new microservice with Docker and create the deployment pipeline",
    "Warp 任意内容，例如：Find and fix the memory leak in my Node.js application",
    "Warp 任意内容，例如：Create a backup script for my PostgreSQL database and schedule it",
    "Warp 任意内容，例如：Help me migrate my data from MySQL to PostgreSQL",
    "Warp 任意内容，例如：Set up monitoring and alerts for my AWS infrastructure",
    "Warp 任意内容，例如：Build a REST API for my mobile app using FastAPI",
    "Warp 任意内容，例如：Help me optimize my SQL queries that are running slowly",
    "Warp 任意内容，例如：Create a GitHub Actions workflow to automatically deploy on merge",
    "Warp 任意内容，例如：Set up Redis caching for my web application",
    "Warp 任意内容，例如：Help me troubleshoot why my Kubernetes pods keep crashing",
    "Warp 任意内容，例如：Build a data pipeline to process CSV files and load them into BigQuery",
    "Warp 任意内容，例如：Set up SSL certificates and configure HTTPS for my domain",
    "Warp 任意内容，例如：Help me refactor this legacy code to use modern design patterns",
    "Warp 任意内容，例如：Create unit tests for my authentication service",
    "Warp 任意内容，例如：Set up log aggregation with ELK stack for my distributed system",
    "Warp 任意内容，例如：Help me implement OAuth2 authentication in my Express.js app",
    "Warp 任意内容，例如：Optimize my Docker images to reduce build times and size",
    "Warp 任意内容，例如：Set up A/B testing infrastructure for my web application",
];
